...
```bash
# mail.search
mu find '<query>' --format=json -n 5

# mail.search_from
mu find 'from:<sender> <query>' --format=json -n 5

# mail.search_to
mu find 'to:<recipient> <query>' --format=json -n 5

# mail.search_cc
mu find 'cc:<recipient> <query>' --format=json -n 5

# mail.search_subject
mu find 'subject:"<subject>" <query>' --format=json -n 5

# mail.search_body
mu find 'body:"<keyword>" <query>' --format=json -n 5

# mail.search_msgid
mu find 'msgid:<message_id>' --format=json -n 5

# mail.search_maildir
mu find 'maildir:/<folder> <query>' --format=json -n 5

# mail.search_unread
mu find 'flag:unread <query>' --format=json -n 5

# mail.search_flagged
mu find 'flag:flagged <query>' --format=json -n 5

# mail.search_attachments
mu find 'flag:attach <query>' --format=json -n 5

# mail.search_date_range
mu find 'date:<start>..<end> <query>' --format=json -n 5

# mail.search_since
mu find 'date:<start>..now <query>' --format=json -n 5

# mail.search_before
mu find 'date:2000-01-01..<end> <query>' --format=json -n 5

# mail.search_latest
mu find '<query>' --sortfield=date --reverse --format=json -n 5

# mail.search_thread
mu find 'subject:"<subject>"' --sortfield=date --reverse --format=json -n 20
```
...
# mail.send_rfc822
msmtp -a default <to> < /tmp/mail.eml

# mail.send_check_only / diag.msmtp_serverinfo
msmtp -a default --serverinfo

# mail.send_himalaya (recommended, keeps Sent folder consistent)
...
若 `msmtp` 报错 `support for authentication method NTLM is not compiled in`，说明当前 msmtp 二进制不支持 NTLM；建议切换到 `mail.send_himalaya`，或将 msmtp 认证改为服务端支持的机制后再重试。

注意：

- `msmtp -P` 等价 `--pretend`，仅打印配置，不会真实发送。
- 如果使用 `--host/--port`，msmtp 会使用命令行临时账户配置；未补齐认证参数时可能报 `530 Authentication required`。
